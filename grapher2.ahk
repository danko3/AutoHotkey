...
;Grid options
   ...
   xScl := %OptPre%xScl ? %OptPre%xScl : Width/10   ; 1

   ;scale of grid for the y plane
   yScl := %OptPre%yScl ? %OptPre%yScl : Height/10  ; 1

   ;lowest x-value shown on graph
   xMin := %OptPre%xMin ? %OptPre%xMin : 0  ; -10

   ;lowest y-value shown on graph
   yMin := %OptPre%yMin ? %OptPre%yMin : 0  ; -10

   ;highest x-value shown on graph
   xMax := %OptPre%xMax ? %OptPre%xMax : Width  ; 10

   ;highest y-value shown on graph
   yMax := %OptPre%yMax ? %OptPre%yMax : Height  ; 10
   ;sanity checks
...
tic          = 5
longtic      = 10
ylabel       := round(ydif/Height,4)
Y_Pos_top_X_Scale_tic := TopMargin-tic
Y_Pos_top_X_Scale_longtic := TopMargin-longtic
Y_Pos_bottom_X_Scale := Height+TopMargin
X_Pos_Left_Y_Scale_tic := LeftMargin-tic
X_Pos_Left_Y_Scale_longtic := LeftMargin-longtic
X_Pos_Right_Y_Scale := Width+LeftMargin
; Labelpositions
Y_Pos_top_X_label := TopMargin-3*longtic
X_Pos_Left_Y_label := LeftMargin-5*longtic
LegendaPos := Height+Topmargin+30
; Plot window size (including borders)
WindowWidth := Width+LeftMargin+50
WindowHeight := Height+TopMargin+BottomMargin
...
Gui Add,text, y%Y_Pos_top_X_Scale_longtic% x%X_Pos_Left_Y_label% w35 +Right, 0
loop, %Height%
{
if !mod(A_Index,10) ; tic every 10 units
{
...
if !mod(A_Index,100) ; label every 100 units
  {
    Y_label := round(A_Index,2)
    Gui Add,text, y%pos% x%X_Pos_Left_Y_label% +Right,%y_label%
  }
}
/*
; Legenda
Gui Add,Progress,h10 w30 y%LegendaPos% x100 backgroundff00000
gui, font, bold cred
Gui Add,text, xp+35 yp-4, chan1
Gui Add,Progress,h10 w30 y%LegendaPos% xp+50 background00aa00
gui, font, c009900
